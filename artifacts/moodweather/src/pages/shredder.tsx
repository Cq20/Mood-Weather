...
const CRISIS_HOTLINE = "400-161-9995"; // 希望24热线（24小时心理危机干预）
...
export default function Shredder() {
  ...
  useEffect(() => {
    track("module_enter", { module: "shredder" });
  }, []);
  ...
  function handleShred() {
    ...
    setPhase("shredding");
    setCrisis(detectCrisis(trimmed));
    recordEvent({
      type: "shredder",
      ts: Date.now(),
              ...
              >
                {crisis ? (
                  <div className="flex flex-col items-center gap-3">
                    <span className="rounded-full bg-white/15 px-3 py-1 text-[11px] text-white/85">
                      你正在经历很难的时候
                    </span>
                    <p className="max-w-xs text-sm leading-7 text-white/95">
                      写下这些，是很勇敢的事。你不必一个人扛着，
                      有人愿意听你说。
                    </p>
                    <a
                      href={`tel:${CRISIS_HOTLINE}`}
                      className="mt-1 rounded-full bg-white/90 px-5 py-2 text-sm font-medium text-slate-700 shadow-sm transition-all duration-200 active:scale-95"
                    >
                      希望24热线 {CRISIS_HOTLINE}
                    </a>
                    <p className="text-[11px] leading-5 text-white/60">
                      24 小时免费心理危机干预热线。我们不是专业心理服务，
                      若你感到危险，请务必联系专业帮助。
                    </p>
                  </div>
                ) : (
          ...
          {phase === "done" ? (
            <>
              ...
              <Link
                ...
                className="inline-flex min-h-10 items-center gap-1.5 rounded-full bg-white/15 px-4 text-xs text-white/85 backdrop-blur-md transition-colors hover:bg-white/25"
              >
                <BookHeart size={14} strokeWidth={1.8} />
                看看我这几天的记录
              </Link>
            </>
          ) : (
