...
export default function Bubble() {
  ...
  useEffect(() => {
    ...
    // 老内核（部分 WebView/旧浏览器）缺少 ResizeObserver 时回退到 window resize
  ...
  function handleRecord() {
    ...
    const rolesData = roles.map((role) => ({
      label: role.label,
      distance: Math.round((distances[role.id]?.ratio ?? 0) * 100) / 100,
    }));
    recordEvent({
      type: "bubble",
      ts: Date.now(),
            ...
            {recorded ? (
              ...
              <div className="flex flex-col items-center gap-2 text-center">
                <p className="text-xs leading-5 text-foreground/55">
                  把此刻的关系距离记下来，过段时间回来看它的变化。
                </p>
                <button
                  type="button"
                  onClick={handleRecord}
                  className="inline-flex min-h-11 items-center gap-2 rounded-full bg-foreground/85 px-5 text-sm font-medium text-white transition-all duration-200 active:scale-[0.98]"
                >
                  <BookHeart size={15} strokeWidth={1.8} />
                  记入日记
                </button>
              </div>
            )}
